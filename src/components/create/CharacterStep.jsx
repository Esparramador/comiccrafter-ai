...
export default function CharacterStep({ characters, setCharacters }) {
  ...
  const removePhoto = (charIndex, photoIndex) => {
    const updated = [...characters];
    const photos = [...(updated[charIndex].photo_urls || [])];
    photos.splice(photoIndex, 1);
    updated[charIndex] = { ...updated[charIndex], photo_urls: photos, photo_url: photos[0] || "" };
    setCharacters(updated);
  };
    ...
    >
      <div className="text-center mb-6">
        ...
        <p className="text-gray-400 text-sm max-w-lg mx-auto">
          Sube varias fotos de referencia por personaje para mayor realismo, o elige de tu biblioteca guardada.
        </p>
      </div>
      ...
      <AnimatePresence>
        {showLibrary && (
          ...
          >
            {savedChars.map(sc => {
              const added = isInList(sc);
              const photos = sc.photo_urls || [];
              return (
                <button
                  key={sc.id}
                  onClick={() => addFromLibrary(sc)}
                  disabled={added}
                  className={`relative flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                    added
                      ? "border-violet-500/40 bg-violet-500/10 opacity-60 cursor-default"
                      : "border-white/5 bg-white/[0.02] hover:border-violet-500/30 hover:bg-violet-500/5"
                  }`}
                >
                  <div className="w-10 h-10 rounded-lg overflow-hidden bg-white/5 flex-shrink-0">
                    {photos[0] ? <img src={photos[0]} alt="" className="w-full h-full object-cover" /> : <UserCircle className="w-6 h-6 text-gray-600 m-auto mt-1.5" />}
                  </div>
                  ...
                  {added && <Check className="w-4 h-4 text-violet-400 absolute top-2 right-2 flex-shrink-0" />}
                </button>
              );
            })}
          </motion.div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-medium text-violet-400 uppercase tracking-wider">Personaje {index + 1}</span>
              </div>
              ...
              <div className="space-y-3">
                ...
                {/* Photos row */}
                <div>
                  <p className="text-xs text-gray-500 mb-2">Fotos de referencia — añade varias para mejor resultado</p>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="w-16 h-16 rounded-lg border-2 border-dashed border-white/10 hover:border-violet-500/40 flex flex-col items-center justify-center text-gray-500 hover:text-violet-400 transition-all"
                    >
                    ...
                    <input
                      ...
                      className="hidden"
                      onChange={(e) => {
                        if (e.target.files?.length) handlePhotoUpload(index, e.target.files);
                        e.target.value = "";
                      }}
                    />
                  </div>
