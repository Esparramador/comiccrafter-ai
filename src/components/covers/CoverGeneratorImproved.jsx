...
import { ImageIcon, Zap, Loader2, Download, RefreshCw, Upload, X, MessageCircle } from "lucide-react";
import LanguageSelector from "../create/LanguageSelector";
...
export default function CoverGeneratorImproved() {
  ...
  const handleModification = async (comments) => {
    ...
    try {
      const categoryPrompts = {
        comic: "professional comic book cover",
        videogame: "epic video game box art",
        clothing: "fashion campaign cover",
        movie: "cinematic movie poster",
        music: "album cover art",
        book: "professional book cover",
        brand: "brand identity cover",
        event: "event poster design",
      };

      const prompt = `Professional ${selectedCat?.label} cover for "${title}". ${selectedStyle?.prompt}. ${categoryPrompts[category]}. Description: ${description}. User modification: ${comments}. Include title text "${title}" prominently. Ultra high quality, 8k resolution, professional design.`;
      ...
      console.error("Error modifying cover:", error);
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-20 bg-gradient-to-br from-[#0a0a0f] to-[#1a1a2e]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            {/* Preview */}
            <div className="rounded-2xl border border-white/5 bg-white/[0.02] overflow-hidden flex items-center justify-center min-h-[400px] relative">
